...
|           | file                 |                  |                            |
+-----------+----------------------+------------------+----------------------------+
|  10       | Syncs and validates  |                  |                            |
|           |                      |                  |                            | 
|           |                      |                  |                            |
+-----------+----------------------+------------------+----------------------------+
|  11       |                      |                  |      Check shared link     |
|           |                      |                  |                            |
+-----------+----------------------+------------------+----------------------------+
|  12       | Final step           | Final step       |        Final Step          |
+-----------+----------------------+------------------+----------------------------+
...
def user3_worker(step):
    ...
    step (11, 'user3 (external viewer) checks shared link')
    
    shared = reflection.getSharedObject()
    url = shared['link']
    download_file(url + '/download', '/tmp/cosas/folder.zip') 

    returncode = runcmd('unzip /tmp/cosas/folder.zip')

    logger.info('returncode of the unzip operation: %s', returncode)

    step (12, 'user3 (external viewer) final step')
