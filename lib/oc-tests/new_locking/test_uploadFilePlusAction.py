...
testsets = [
        { 'action_method': 'put_file_contents',
          'action_args': ('/folder/new_bigfile.dat', '123'*50),
          'action_kwargs': {},
          'accounts': sconf.oc_number_test_users,
          'extra_check': 'check_filesize',
          'extra_check_params': ('/folder/new_bigfile.dat', 3*50),
...
#        },
        { 'action_method': 'move',
          ...
          'extra_check_params': (('/folder-renamed', '/folder-renamed/bigfile.dat', '/folder-renamed/new_bigfile.dat'),
                                    ('/folder', '/folder/bigfile.dat', '/folder/new_bigfile.dat')),
          'overwrite_kwargs' : {'chunked': False},
        },
        { 'action_method': 'move',
          ...
          'accounts': sconf.oc_number_test_users,
          'extra_check': 'check_first_list_exists_second_list_not',
          'extra_check_params': (('/folder2', '/folder2/folder', '/folder2/folder/bigfile.dat', '/folder2/folder/new_bigfile.dat'),
                                    ('/folder', '/folder/bigfile.dat', '/folder/new_bigfile.dat')),
          'overwrite_kwargs' : {'chunked': False},
        },
]
...
def uploader(step):
    ...
    try:
        ...
        get_file_result = client_wrapper.do_action('get_file', '/folder/new_bigfile.dat', tmpfile2[1])

        sum_downloaded = md5sum(tmpfile2[1])

        # check both md5 matches
        error_check(get_file_result, 'uploaded file failed to download')
...
def doer(step):
    ...
    args = config.get('action_args', ('/folder/new_bigfile.dat', '123'*50))
...
for i in range(1, config.get('accounts', 1) + 1):
    add_worker(uploader, name='uploader_%s' % (i,))
